...
def show_general_settings(master, config: dict, style, theme_func):
    ...
    # -------------------- state + helpers --------------------
    # Simplex API (default OFF)
    enable_simplex_var = tk.BooleanVar(value=bool(config.get("enable_simplex_api", False)))
    ...
    # thread count (parallelism) – clamp to CPU count
    ...
    # originals for "Save" enable logic
    original_simplex  = enable_simplex_var.get()
    ...
    def enable_save_btn_if_changed(*_):
        changed = (
            enable_simplex_var.get()        != original_simplex
            or working_dir_var.get()        != original_wd
    ...
    # ===================== Simplex API (TOP) =====================
    frame_simplex = ttk.LabelFrame(win, text="Simplex API (external control)", padding=10)
    frame_simplex.pack(fill="x", padx=16, pady=(12, 8))

    ttk.Label(
        frame_simplex,
        text="When enabled, this API may override parts of the GUI state (e.g., active compile mode).",
        wraplength=540,
        foreground="#666",
        justify="left",
    ).grid(row=0, column=0, columnspan=2, sticky="w", pady=(0, 6))

    chk_simplex = ttk.Checkbutton(
        frame_simplex,
        text="Enable Simplex API (may override GUI state)",
        variable=enable_simplex_var,
        onvalue=True, offvalue=False,
        command=enable_save_btn_if_changed,
    )
    chk_simplex.grid(row=1, column=0, sticky="w")

    # ===================== Parallel Build Threads =====================
    frame_threads = ttk.LabelFrame(win, text="Parallel Build Threads", padding=10)
    frame_threads.pack(fill="x", padx=16, pady=(0, 8))
    frame_threads.columnconfigure(1, weight=0)

    ttk.Label(
        frame_threads,
        text=("Sets the number of worker threads used during ‘Compile All’ when Top/Down Pipeline Mode is disabled. "
    ...
    threads_spin = _Spinbox(
        frame_threads, from_=1, to=_cpu_max, increment=1,
        textvariable=thread_count_var, width=7, justify="center", state="normal", wrap=False,
    )
    threads_spin.grid(row=1, column=1, sticky="w")
    ttk.Label(frame_threads, text=f"Detected CPU cores: {_cpu_max}", foreground="#666").grid(row=1, column=2, sticky="w", padx=(8, 0))
    ...
    ttk.Label(
        frame_hash,
        text=("Before ‘Compile All’, verifies that the local toolchain matches the developer-approved "
    ...
    chk_hash = ttk.Checkbutton(
        ...
        variable=enable_hashcheck_var, onvalue=True, offvalue=False,
        command=enable_save_btn_if_changed,
    )
    ...
    ttk.Label(
        frame_seq,
        text=("When enabled, 'Compile All' creates a top/down pipeline: no parallel exports. "
              "All projects run strictly from top to bottom (A/B/C) exactly as shown in the list. "
    ...
    chk_seq = ttk.Checkbutton(
        ...
        variable=sequential_build_var, onvalue=True, offvalue=False,
        command=enable_save_btn_if_changed,
    )
    ...
    _Spinbox2 = getattr(ttk, "Spinbox", tk.Spinbox)
    cooldown_spin = _Spinbox2(
        frame_seq, from_=0, to=86400, increment=1,
        textvariable=pipeline_cooldown_var, width=7, justify="center", state="normal", wrap=False,
    )
    ...
    def save():
        ...
        # persist
        config["enable_simplex_api"] = bool(enable_simplex_var.get())  # NEW
        ...
        if hasattr(master, "enable_hashcheck_var"):
            try: master.enable_hashcheck_var.set(bool(enable_hashcheck_var.get()))
            except Exception: pass
        if hasattr(master, "sequential_build_var"):
            try: master.sequential_build_var.set(bool(sequential_build_var.get()))
            except Exception: pass
        try:
        ...
        if hasattr(master, "pipeline_cooldown_s"):
            try: master.pipeline_cooldown_s = cooldown_val
            except Exception: pass

        # Simplex watcher live toggle
        try:
            if hasattr(master, "enable_simplex_api_var"):
                master.enable_simplex_api_var.set(bool(enable_simplex_var.get()))
            # stop existing watcher if any
            if getattr(master, "_simplex_watcher", None):
                master._simplex_watcher.stop()
                master._simplex_watcher = None
            # start new watcher only when enabled
            if enable_simplex_var.get():
                ini_path = (master.working_dir / "simplexAPI.ini")
                from .simplex_api import SimplexAPIWatcher
                master._simplex_watcher = SimplexAPIWatcher(master, ini_path, poll_interval=1.0)
                master._simplex_watcher.start()
        except Exception:
            pass

        messagebox.showinfo(
            "Saved",
            f"Simplex API: {'enabled' if enable_simplex_var.get() else 'disabled'}\n"
            ...
            "(A restart might be necessary if other modules cache settings during import.)",
        )
        win.destroy()
    ...
    enable_simplex_var.trace_add("write", enable_save_btn_if_changed)
