...
try:
    from conf_gui_overrides import *  # noqa: F401,F403
except Exception:
    pass
{HOOK_END}
"""
...
def ensure_gui_hook(conf_path: str):
    """Append (idempotently) the import hook to the end of conf.py."""
...
def write_conf_overrides(conf_dir: str, overrides: dict) -> str:
    """Write conf_gui_overrides.py with only keys from `overrides`.
    Complex types are written via pformat so the file is valid Python.
    """
    os.makedirs(conf_dir, exist_ok=True)
    out_path = os.path.join(conf_dir, "conf_gui_overrides.py")

    lines = ["# Auto-generated by SphinxEditor GUI.\n",
...
def build_sphinx(project):
    ...
    Uses -D only for simple scalars (project, author, html_theme). Complex types
    (extensions, html_theme_options, language, release) should go via the overlay file.
    """
...
class SphinxEditor:
    ...
    def show(self) -> bool:
        ...
        sep.grid(row=5, column=0, columnspan=4, sticky="ew", pady=(8, 8))

        ttk.Label(form, text="Project:").grid(row=6, column=0, sticky="e", padx=5, pady=4)
        self.e_proj = ttk.Entry(form, width=45)
        self.e_proj.grid(row=6, column=1, padx=5, pady=4, sticky="ew")
        self.e_proj.insert(0, getattr(self.project, "sphinx_project", "Your Project"))

        ttk.Label(form, text="Author:").grid(row=7, column=0, sticky="e", padx=5, pady=4)
        self.e_author = ttk.Entry(form, width=45)
        self.e_author.grid(row=7, column=1, padx=5, pady=4, sticky="ew")
        self.e_author.insert(0, getattr(self.project, "sphinx_author", "Your Name"))

        ttk.Label(form, text="Release:").grid(row=8, column=0, sticky="e", padx=5, pady=4)
        self.e_release = ttk.Entry(form, width=45)
        self.e_release.grid(row=8, column=1, padx=5, pady=4, sticky="ew")
        self.e_release.insert(0, getattr(self.project, "sphinx_release", ""))

        ttk.Label(form, text="Language (e.g. en, de):").grid(row=9, column=0, sticky="e", padx=5, pady=4)
        self.e_lang = ttk.Entry(form, width=45)
        self.e_lang.grid(row=9, column=1, padx=5, pady=4, sticky="ew")
        self.e_lang.insert(0, getattr(self.project, "sphinx_language", ""))

        # --- Theme ---
        ttk.Label(form, text="Theme:").grid(row=10, column=0, sticky="e", padx=5, pady=4)
        self.theme_var = tk.StringVar()
        themes = get_installed_themes()
        self.theme_combo = ttk.Combobox(
            form, textvariable=self.theme_var, values=themes, width=26, state="readonly"
        )
        self.theme_combo.grid(row=10, column=1, sticky="w", padx=5, pady=4)
        ...
        # Button NACH der Funktionsdefinition anlegen
        help_btn = ttk.Button(form, text="Help…", command=show_theme_help)
        help_btn.grid(row=10, column=2, padx=5)




        # --- Extensions (one per line) ---
        ttk.Label(form, text="Extensions (one per line):").grid(row=11, column=0, sticky="ne", padx=5, pady=4)
        self.txt_ext = scrolledtext.ScrolledText(form, width=45, height=5, font=("Segoe UI", 10))
        self.txt_ext.grid(row=11, column=1, padx=5, pady=4, sticky="ew")
        # Picker button
        ttk.Button(form, text="Pick…", command=self._pick_extensions_dialog)\
            .grid(row=11, column=2, padx=5, sticky="n")

        # --- Theme options (JSON or Python literal) ---
        ttk.Label(form, text="html_theme_options (JSON/Python):").grid(row=12, column=0, sticky="ne", padx=5, pady=4)
        self.txt_theme_opts = scrolledtext.ScrolledText(form, width=45, height=5, font=("Segoe UI", 10))
        self.txt_theme_opts.grid(row=12, column=1, padx=5, pady=4, sticky="ew")
        ...
        # --- Parallel jobs & flags ---
        ttk.Label(form, text="Parallel jobs (-j):").grid(row=13, column=0, sticky="e", padx=5, pady=4)
        self.e_parallel = ttk.Entry(form, width=8)
        self.e_parallel.grid(row=13, column=1, sticky="w", padx=5, pady=4)
        self.e_parallel.insert(0, str(getattr(self.project, "sphinx_parallel", 1)))

        cb_frame = ttk.Frame(form)
        cb_frame.grid(row=14, column=0, columnspan=4, sticky="w", pady=(6, 0))
        ...
        # --- Additional args ---
        ttk.Label(form, text="Additional Sphinx arguments:").grid(row=15, column=0, sticky="ne", padx=5, pady=4)
        self.txt_args = scrolledtext.ScrolledText(form, width=45, height=3, font=("Segoe UI", 10))
        self.txt_args.grid(row=15, column=1, padx=5, pady=4, sticky="ew")
        self.txt_args.insert(tk.END, " ".join(getattr(self.project, "sphinx_args", [])))

        # --- Buttons ---
        button_frame = ttk.Frame(form)
        button_frame.grid(row=16, column=0, columnspan=4, pady=16)
    ...
    def _load_conf_namespace(self, conf_path: str) -> dict | None:
        """Execute conf.py in an isolated namespace, masking conf_gui_overrides."""
        if not conf_path or not os.path.isfile(conf_path):
            return None
        conf_dir = os.path.dirname(os.path.abspath(conf_path))
        old_cwd = os.getcwd()
        had_ov = "conf_gui_overrides" in importlib.sys.modules
        old_ov = importlib.sys.modules.get("conf_gui_overrides")
        # Mask overrides so we get base conf values
        importlib.sys.modules["conf_gui_overrides"] = types.ModuleType("conf_gui_overrides")
        try:
            os.chdir(conf_dir)
            ...
            # Restore masking
            if had_ov:
                importlib.sys.modules["conf_gui_overrides"] = old_ov  # type: ignore
            else:
                import importlib as _il
                try:
                    del _il.sys.modules["conf_gui_overrides"]
                except KeyError:
                    pass
            os.chdir(old_cwd)
    ...
    def _save(self):
        ...
        self.project.sphinx_args = shlex.split(args) if args else []
