...
def _strip_pack_options(args: list[str]) -> list[str]:
    """
    Entfernt ALLE Pack-/PyInstaller-bezogenen Flags aus der Argumentliste.
    Ziel: reine 'gen'-Aufrufe ohne Alt-Embed oder PyInstaller-Müll.
    """
    out: list[str] = []
    i = 0
    while i < len(args):
        ...
        # PyArmor-Pack-Schalter komplett entfernen
        if tok == "--pack":
            i += 1
            # evtl. Modus konsumieren
        ...
        # Alt-Embed / in v9 ist -e == --expired → hier grundsätzlich nicht akzeptieren
        if tok == "-e":
        ...
        # PyInstaller-Top-Level-Flags entfernen
        if tok in ("--onefile", "--onedir", "--windowed", "--console"):
            i += 1
            continue

        # PyInstaller-Pfade/Icon entfernen (mit Wert)
...
class CPB0000000:
    ...
    def run_pyarmor(project: Project, log_file) -> None:
        ...
        # 7) Ausführung (keine BCC/Themida/Pro-Umgebung mehr)
        try:
